...
def expand_model(strategy, integration, integration_length, n_add,
                 model: MemTransformerLM, optimizers, schedulers, tr_iter,
                 va_iter, epoch, step):
    ...
    if "reverse_distil" in integration:
        fit_to_previous_model(model, new_layers, tr_iter,
                              first_logits, integration)
...
def expand_state(param, state):
    if param.shape != state.shape:
        ratios = [param.shape[i] // state.shape[i]
...
def widen_model(strategy, ratio, model: MemTransformerLM, optimizers,
                va_iter, epoch, step):
    optimizer, _ = optimizers
...
if __name__ == "__main__":
    ...
    logging = create_exp_dir(args.work_dir,
                             scripts_to_save=['train_ts.py', 'mem_transformer.py'], debug=args.debug)
